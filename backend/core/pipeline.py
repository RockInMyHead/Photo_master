...
async def process_folder(
    ...
    *,
    ...
    # СБАЛАНСИРОВАННЫЕ параметры: один человек = одна папка, без лишних разбиений
    link_sim = 0.58      # Мягче - объединяет вариации одного человека
    merge_sim = 0.75     # Мягче - объединяет кластеры одного человека
    assign_sim = 0.65    # Умеренно - добавляет outliers если уверенно
    min_intra_sim = 0.52 # Умеренно - не выбрасывает вариации одного лица
    assign_margin = 0.05 # Умеренно - требует небольшого отрыва от второго
