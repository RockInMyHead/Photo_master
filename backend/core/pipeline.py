...
async def process_folder(
    ...
    *,
    ...
    # МАКСИМАЛЬНО СТРОГИЕ параметры для избежания дубликатов
    link_sim = 0.70      # Выше - только очень похожие лица объединяются
    merge_sim = 0.85     # Выше - кластеры объединяются только при высокой схожести
    assign_sim = 0.72    # Выше - строгий порог для назначения в кластер
    min_intra_sim = 0.60 # Выше - строже чистка внутри кластера
    assign_margin = 0.06 # Выше - больше разница между кандидатами нужна
